...
def my_products_view(request):
    # İstifadəçinin əlavə etdiyi məhsulları əldə et
    mehsullar = Mehsul.objects.filter(user=request.user).order_by('-id')
...
def my_orders_view(request):
    # İstifadəçiyə gələn sifarişləri əldə et
    sifarisler = []
    user_products = Mehsul.objects.filter(user=request.user)
    
    for product in user_products:
        order_items = SifarisItem.objects.filter(mehsul=product)
        for item in order_items:
            if item.sifaris not in sifarisler:
                sifarisler.append(item.sifaris)
...
def add_product_view(request):
    if request.method == 'POST':
        # Məhsul əlavə etmə məntiqini burada yazın
        try:
            yeni_mehsul = Mehsul(
                user=request.user,
            ...
            if 'sekil' in request.FILES:
                yeni_mehsul.sekil = request.FILES['sekil']
            ...
            messages.error(request, 'Məhsul əlavə edilərkən xəta baş verdi!')
    
    # Forma üçün lazım olan məlumatları göndər
    kateqoriyalar = Kateqoriya.objects.all()
    firmalar = Firma.objects.all()
    avtomobiller = Avtomobil.objects.all()
