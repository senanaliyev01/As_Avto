...
@login_required
def my_firmalar(request):
    firmalar = Firma.objects.filter(id__in=Mehsul.objects.filter(sahib=request.user).values_list('firma', flat=True).distinct())
    return render(request, 'my_firmalar.html', {'firmalar': firmalar})

@login_required
def my_kateqoriyalar(request):
    kateqoriyalar = Kateqoriya.objects.filter(id__in=Mehsul.objects.filter(sahib=request.user).values_list('kateqoriya', flat=True).distinct())
    return render(request, 'my_kateqoriyalar.html', {'kateqoriyalar': kateqoriyalar})

@login_required
def my_avtomobiller(request):
    avtomobiller = Avtomobil.objects.filter(id__in=Mehsul.objects.filter(sahib=request.user).values_list('avtomobil', flat=True).distinct())
    return render(request, 'my_avtomobiller.html', {'avtomobiller': avtomobiller})