...
def products_list(request):
    ...
    if search_text:
        ...
        mehsullar = mehsullar.filter(
            ...
            Q(brend_kod__icontains=search_text)  # Brend kodu ilə axtarış
        ).distinct()
...
def mehsul_axtaris(request):
    ...
    if query:
        ...
        mehsullar = mehsullar.filter(
            ...
            Q(brend_kod__icontains=query)  # Brend kodu ilə axtarış
        ).distinct()
...
def generate_pdf(sifaris, sifaris_mehsullari, profile):
    ...
    for index, mehsul in enumerate(sifaris_mehsullari, start=1):
        data.append([index, mehsul.mehsul.adi, mehsul.mehsul.brend.adi, mehsul.mehsul.oem, mehsul.miqdar, f"{mehsul.qiymet} AZN", f"{mehsul.cemi} AZN"])
