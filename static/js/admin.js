...
// Məhsullar cədvəli funksiyaları
document.addEventListener('DOMContentLoaded', function() {
    initializeMehsulTable();
});

function initializeMehsulTable() {
    const table = document.querySelector('.mehsul-table');
    if (!table) return;
    ...
    // Qiymət və stok sütunlarını formatla
    const priceFields = table.querySelectorAll('.field-qiymet, .field-maya_qiymet');
    priceFields.forEach(field => {
        const value = field.textContent.trim();
        if (!isNaN(value)) {
            field.textContent = parseFloat(value).toFixed(2) + ' ₼';
        }
    });

    const stockFields = table.querySelectorAll('.field-stok');
    ...
    // Uzun məhsul adları üçün tooltip
    const nameFields = table.querySelectorAll('.field-adi');
    nameFields.forEach(field => {
        const link = field.querySelector('a');
        if (link && link.offsetWidth < link.scrollWidth) {
            link.title = link.textContent;
        }
    });
}

// Filtr panelini yığcamlaşdır
function toggleFilters() {
    const filterContainer = document.querySelector('.mehsul-filters');
    if (filterContainer) {
