    ...
    // Azərbaycan hərflərini normallaşdırmaq üçün funksiya
    function normalizeSearchText(text) {
        ...
        for (let [az, en] of Object.entries(azToEn)) {
            text = text.replace(new RegExp(az, 'g'), en);
        }
        
        // Yalnız hərf və rəqəmləri saxla
    ...
    function displaySearchResults(results, searchTerm) {
        ...
        const resultsHtml = results.map(mehsul => {
            const stockStatus = getStockStatus(mehsul.stok);
            const stockClass = getStockStatusClass(mehsul.stok);
            const highlightedName = highlightSearchTerm(mehsul.adi, searchTerm);
            const highlightedOem = highlightSearchTerm(mehsul.oem || '', searchTerm);
            const highlightedBrendKod = highlightSearchTerm(mehsul.brend_kod || '', searchTerm);
            
            return `
                <div class="search-result-item" onclick="window.location.href='/mehsullar/mehsul/${mehsul.id}/${encodeURIComponent(mehsul.adi)}/${encodeURIComponent(mehsul.oem)}/${encodeURIComponent(mehsul.brend_kod)}'">
                    <img src="${mehsul.sekil_url || '/static/img/no-image.png'}" alt="${mehsul.adi}">
                    <div class="search-result-info">
                        <h4>${highlightedName}</h4>
                        <p>Firma: ${mehsul.brend}</p>
                        <p>Model: ${mehsul.marka}</p>
                        <p>OEM: ${highlightedOem}</p>
                        <p>Brend Kodu: ${highlightedBrendKod}</p>
                    </div>
                    <div class="search-result-price">
                        <span>${mehsul.qiymet} AZN</span>
    ...
    // Stok statusunu təyin etmək üçün funksiya
    function getStockStatus(stok) {
        if (stok === 0) return "Yoxdur";
        if (stok <= 20) return "Az var";
        return "Var";
    }

    // Stok status klassını təyin etmək üçün funksiya
    function getStockStatusClass(stok) {
        if (stok === 0) return "out-of-stock";
        if (stok <= 20) return "low-stock";
        return "in-stock";
    }

    // Debounce funksiyasını təkmilləşdirək
    function debounce(func, wait) {
        let timeout;
        return function executedFunction(...args) {
            const later = () => {
    ...
    async function performSearch() {
        ...
        if (searchTerm.length < 2 && !category && !brand && !model) {
            dropdownContainer.classList.remove('active');
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`/mehsullar/realtime-search/?${params}`);
            const data = await response.json();
    ...
    // Event listener-ləri əlavə edək
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Dropdown container yaratmaq
        if (!dropdownContainer) {
            const newDropdown = document.createElement('div');
            newDropdown.className = 'search-results-dropdown';
            searchForm.appendChild(newDropdown);
        }
        
        // Axtarış inputuna event listener əlavə et
        const debouncedSearch = debounce(performSearch, 300);
        searchInput.addEventListener('input', debouncedSearch);
        categorySelect?.addEventListener('change', debouncedSearch);
        brandSelect?.addEventListener('change', debouncedSearch);
        modelSelect?.addEventListener('change', debouncedSearch);
